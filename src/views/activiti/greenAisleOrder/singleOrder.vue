<template>
  <!-- 单笔指令 -->
  <div>
    <a-card :bordered="false">
      <div class="table-page-search-wrapper">
        <a-form-model class="ant-advanced-search-form" layout="inline" :model="formData" ref="ruleForm" :rules="rules">
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="指令来源"
                prop="channel"
                :rules="{
                  required: true,
                  message: '请选择指令来源',
                  trigger: 'change',
                }"
              >
                <a-select v-model="formData.channel" placeholder="请输入指令来源" allowClear>
                  <a-select-option v-for="v in channel" :key="v.value">{{ v.text }}</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="付款类型"
                prop="transferType"
                :rules="{
                  required: true,
                  message: '请选择付款类型',
                  trigger: 'change',
                }"
              >
                <a-radio-group v-model="formData.transferType" allowClear>
                  <a-radio value="1"> 行内转账</a-radio>
                  <a-radio value="2"> 跨行转账</a-radio>
                  <a-radio value="3"> 智能支付</a-radio>
                </a-radio-group>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="指令类型"
                prop="caseType"
                :rules="{
                  required: true,
                  message: '请选择指令类型',
                  trigger: 'change',
                }"
              >
                <a-select v-model="formData.caseType" placeholder="请输入指令类型" allowClear>
                  <a-select-option v-for="v in caseType" :key="v.value">{{ v.text }}</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="划款转账"
                prop="payDate"
                :rules="{
                  required: true,
                  message: '请选择时间',
                  trigger: 'change',
                }"
              >
                <a-date-picker
                  v-model="formData.payDate"
                  format="YYYY-MM-DD"
                  :disabled-date="disabledDate"
                  placeholder="请选择时间"
                  allowClear
                  style="width: 100%"
                />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="业务类型"
                prop="bizTypeCode"
                :rules="{
                  required: true,
                  message: '请选择业务类型',
                  trigger: 'change',
                }"
              >
                <a-select v-model="formData.bizTypeCode" placeholder="请输入业务类型" allowClear>
                  <a-select-option v-for="v in bizTypeCode" :key="v.value">{{ v.text }}</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="币种"
                prop="currency"
                :rules="{
                  required: true,
                  message: '请选择币种',
                  trigger: 'change',
                }"
              >
                <a-select v-model="formData.currency" placeholder="请选择币种" allowClear>
                  <a-select-option v-for="item in currencyList" :key="item.index" :value="item">
                    {{ item }}
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="项目名称"
                prop="projectName"
                :rules="{
                  required: true,
                  message: '请输入项目名称',
                  trigger: 'change',
                }"
              >
                <a-row :gutter="14">
                  <a-col :span="23">
                    <a-input v-model="formData.projectName" placeholder="请输入项目名称" allowClear />
                  </a-col>
                  <a-col :span="1">
                    <a-button type="primary">查询项目</a-button>
                  </a-col>
                </a-row>
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="金额"
                prop="amount"
                :rules="{
                  required: true,
                  message: '请输入金额',
                  trigger: 'change',
                }"
              >
                <a-input-number
                  style="width: 100%"
                  v-model="formData.amount"
                  placeholder="请输入金额"
                  :precision="4"
                  :min="0"
                  :max="20"
                  allowClear
                >
                </a-input-number>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="客户名称"
                prop="customerName"
                :rules="{
                  required: true,
                  message: '请输入客户名称',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.customerName" placeholder="请输入客户名称" allowClear />
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="大写金额"
                prop="capitalAmount"
                :rules="{
                  required: true,
                  message: '请输入大写金额',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.capitalAmount" placeholder="请输入大写金额" allowClear />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="付款账号"
                prop="payAccount"
                :rules="{
                  required: true,
                  message: '请输入付款账号',
                  trigger: 'change',
                }"
              >
                <a-row :gutter="24">
                  <a-col :span="15">
                    <a-input v-model="formData.payAccount" placeholder="请输入付款账号" allowClear />
                  </a-col>
                  <a-col :span="3" pull="1">
                    <a-button type="primary">查询付款方</a-button>
                  </a-col>
                  <a-col :span="3" push="2">
                    <a-button type="primary">账户余额</a-button>
                  </a-col>
                  <a-col :span="3" push="4">
                    <a-button type="primary">账户明细</a-button>
                  </a-col>
                </a-row>
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item
                label="用途"
                prop="payPurpose"
                :rules="{
                  required: true,
                  message: '请选择用途',
                  trigger: 'change',
                }"
              >
                <a-row :gutter="24">
                  <a-col :span="12">
                    <a-select
                      v-model="formData.payPurpose"
                      palaceholder="自定义"
                      style="width: 200px"
                      allowClear
                      @change="handleChange"
                    >
                      <a-select-option v-for="item in payPurposeList" :key="item.index" :value="item">
                        {{ item }}
                      </a-select-option>
                    </a-select>
                  </a-col>
                  <a-col :span="12">
                    <a-input allowClear placeholder="请输入自定义用途" v-model="formData.useRemark" />
                  </a-col>
                </a-row>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="付款户名"
                prop="payAccountName"
                :rules="{
                  required: true,
                  message: '请输入付款户名',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.payAccountName" placeholder="请输入付款户名" allowClear />
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item label="是否立即划款" prop="isReceipt">
                <a-radio-group v-model="formData.isReceipt" @change="onChange" allowClear>
                  <a-radio value="1"> 是</a-radio>
                  <a-radio value="2"> 否</a-radio>
                </a-radio-group>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="收款账号"
                prop="recvAccount"
                :rules="{
                  required: true,
                  message: '请输入收款账号',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.recvAccount" placeholder="请输入收款账号" allowClear />
              </a-form-model-item>
            </a-col>
            <a-col :span="8" :offset="4">
              <a-form-model-item label="流程备注" prop="transNote">
                <a-input
                  v-model="formData.transNote"
                  type="textarea"
                  placeholder="请输入备注,最多输入100个字符"
                  allowClear
                />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="收款户名"
                prop="recvAccountName"
                :rules="{
                  required: true,
                  message: '请输入收款户名',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.recvAccountName" placeholder="请输入收款户名" allowClear />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="开户行"
                prop="payBankName"
                :rules="{
                  required: true,
                  message: '请输入开户行',
                  trigger: 'change',
                }"
              >
                <a-row :gutter="24">
                  <a-col :span="15">
                    <a-input v-model="formData.payBankName" placeholder="请输入开户行" allowClear />
                  </a-col>
                  <a-col :span="3" pull="1">
                    <a-button type="primary">查询收款行</a-button>
                  </a-col>
                  <a-col :span="3" push="2">
                    <a-button type="primary">手工录入</a-button>
                  </a-col>
                </a-row>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-model-item
                label="大额行号"
                prop="payBankNo"
                :rules="{
                  required: true,
                  message: '请输入大额行号',
                  trigger: 'change',
                }"
              >
                <a-input v-model="formData.payBankNo" placeholder="请输入大额行号" allowClear />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="24">
              <a-button style="float: right; margin-left: 10px" type="primary" @click="handleCancel"> 清空</a-button>
              <a-button style="float: right; margin-left: 10px" type="primary" :loading="loading" @click="handleOk">
                提交
              </a-button>
            </a-col>
          </a-row>
        </a-form-model>
      </div>
    </a-card>
  </div>
</template>

<script>
// import { LeadingtekListMixin } from '@/mixins/LeadingtekListMixin'
import { postAction, deleteAction, getAction } from '@api/manage'
import { singleCase } from '@api/api'
import store from '@/store'
import moment from 'moment';
export default {
  // mixins: [LeadingtekListMixin],
  created: function () {
    store.dispatch('updateCaseSource', this.url.dict + 'case_source').then((e) => {
      console.log(e)
      if (e.success) {
        this.channel = e.result
      }
    })
    store.dispatch('updateCaseType', this.url.dict + 'case_type').then((e) => {
      console.log(e)
      if (e.success) {
        this.caseType = e.result
      }
    })

    store.dispatch('updateCaseBusinessType', this.url.dict + 'case_business_type').then((e) => {
      console.log(e)
      if (e.success) {
        this.bizTypeCode = e.result
      }
    })
  },
  data() {
    return {
      loading: false,
      url: {
        dict: '/sys/dict/getDictItems/', //码表接口地址
      },
      rules: {},
      formData: { transferType: '1',isReceipt: '1' },
      channel: [], //指令来源
      caseType: [], //指令类型
      bizTypeCode: [], //业务类型

      // 币种列表
      currencyList: ['人民币', '美元', '港元', '日元', '欧元', '英镑', '瑞郎', '加元', '澳元', '新加坡元'],
      // 用途列表
      payPurposeList: [
        '投资款',
        '收益分配',
        '到期提取本金及收益',
        '基金申购款',
        '基金赎回款',
        '基金轧差款',
        '管理费',
        '托管费',
        '销售服务费',
        '委贷手续费',
        '财务顾问费',
        '审计费',
        '律师费',
        '税款',
        '销户尾款',
        '其他费用',
        '到期提取',
        '投资顾问费',
        '客户服务费',
        '佣金',
        '银转期',
        '期转银',
        '风险准备金',
        '其他转账',
        '自定义',
      ],
    }
  },
  methods: {
    disabledDate(current) {
      return current && current > moment()
    },
    handleChange(value) {
      console.log(`selected ${value}`)
    },
    onChange(e) {
      console.log('radio checked', e.target.value)
    },
    handleCancel(e) {
      this.$confirm({
        title: '提醒',
        content: '是否清空',
        okText: '确定',
        cancelText: '取消',
      })
    },
    handleOk() {
      // singleCase({}).then((res) => {
      //   console.log(res)
      // })
      console.log(this.formData.payDate.format('YYYY-MM-DD'))
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.loading = true

          singleCase({ ...this.formData, payDate: this.formData.payDate.format('YYYY-MM-DD') })
            .then((res) => {
              console.log(res)
              // if (res.ret == 0) {
              //   this.loading = false
              //   this.handleCancel()
              // } else {
              //   this.$message.warning(res.msg)
              // }
            })
            .finally((end) => {
              this.loading = false
            })
        } else {
          return false
        }
      })
    },
    // this.$confirm({
    //   title: '提醒',
    //   content: '是否提交',
    //   okText: '确定',
    //   cancelText: '取消'
    // })
  },
}
</script>

<style scoped>
.table-page-search-wrapper {
  /* background: red; */
  padding: 30px;
  /* background: red; */
}
</style>